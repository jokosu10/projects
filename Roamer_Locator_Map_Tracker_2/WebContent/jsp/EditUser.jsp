...
<title>Roamer Locator Map Tracker [ Edit User Page ] </title>
<head>
...
$("#document").ready(function() {
	...
	if(sessionId === "" || sessionId == null) {
		...
		if(sessionType !== "2" && sessionType !== "1") {
			...
			if(document.referrer === "") {
				$("#blankDiv").show();
				alert("You cannot access directly this page! Redirecting to UserOperations page...");
				location.href="UserOperations.jsp";
				$("#blankDiv").hide();
			} else {
				processPage();
			}	
   ...
   function processPageFromVar() {
		...
		$.ajax({
	     	type:'POST',
	         url:'insertToUserLogs.action?userId='+sessionId2+'&fullName='+ sessionName2 +'&action=View&page=EditUser.jsp',
		...
		// load values from UserOperations.jsp
		var uId = decodeURIComponent(getParamsFromUrl("userId"));
		$("#userName").val(decodeURIComponent(getParamsFromUrl("userName")));
		$("#fullName").val(decodeURIComponent(getParamsFromUrl("userFullName")));
		// end
			...
			if($("#password").val().length < 8) {
				...
				if($("#password").val() !== $("#confirmPassword").val()) {
					...
					if(missingParam !== "") {
						...
						$.ajax({
							url: "updateUser.action?userId="+ uId +"&userType="+ $('#userTypeDropdown').val() +"&userName="+$('#userName').val()+"&password="+$('#password').val()+"&fullName="+$('#fullName').val()+"&updatedBy="+sessionName2,
							type: "POST",
							contentType: 'application/json; charset=utf-8',
				            dataType: 'json',
							success: function(response){
								var res = JSON.parse(JSON.stringify(response)).updateUserReturn;
								if(res.indexOf("exist") >= 0) {
									...
									if(Boolean(res) == true) {
										...
										$.ajax({
									     	type:'POST',
									         url:'insertToUserLogs.action?userId='+sessionId2+'&fullName='+ sessionName2 +'&action=Edit_User&page=EditUser.jsp',
									         contentType: 'application/json; charset=utf-8',
									         dataType: 'json'
										});
   ...
   function processPage() {
		...
		$.ajax({
	     	type:'POST',
	         url:'insertToUserLogs.action?userId='+sessionId+'&fullName='+ sessionName +'&action=View&page=EditUser.jsp',
	         contentType: 'application/json; charset=utf-8',
	         dataType: 'json'
		});
		...
		$("#userName").val(decodeURIComponent(getParamsFromUrl("userName")));
		$("#fullName").val(decodeURIComponent(getParamsFromUrl("userFullName")));
		$("#userTypeDropdown").val(decodeURIComponent(getParamsFromUrl("userType")));
		// end
			...
			if($("#password").val().length < 8) {
				...
				if($("#password").val() !== $("#confirmPassword").val()) {
					...
					if(missingParam !== "") {
						...
						$.ajax({
							url: "updateUser.action?userId="+ uId +"&userType="+ $('#userTypeDropdown').val() +"&userName="+$('#userName').val()+"&password="+$('#password').val()+"&fullName="+$('#fullName').val()+"&updatedBy="+sessionName,
							type: "POST",
							contentType: 'application/json; charset=utf-8',
				            dataType: 'json',
							success: function(response){
								...
								if(res.indexOf("exist") >= 0) {
									...
									if(Boolean(res) == true) {
										...
										$.ajax({
									     	type:'POST',
									         url:'insertToUserLogs.action?userId='+sessionId+'&fullName='+ sessionName +'&action=Edit_User&page=EditUser.jsp',
									         contentType: 'application/json; charset=utf-8',
									         dataType: 'json'
										});
...
<body>
	...
	<div id="main">
		<div id="map_header">
			...
			<div class="reporting">
				<!--<a onclick="location.href='https://10.8.72.12/RoamerLocatorMapTracker/index.php/admin/InboundRoamersReportViewAndGeneration'" style="text-decoration: none;"/><img src= "../images/3.png" style="margin-top: 50px; margin-left: 15px;"><br><label style="font-family: sans-serif; font-size: small; color: white;">Reporting</label></a>-->
			...
			<br/>
				...
				<div style="float: left; display: inline-block; width: 40%; height: 500px;">
				    ...
				    <div style="position: absolute; margin: 0 auto; width: inherit;">
						<label style="font-size: x-large; color: black; margin-left: 25%;">View Users&nbsp;</label><label style="font-size: x-large; color: black; margin-left: 5px;">>>&nbsp;</label><label style="font-size: x-large; color: blue; margin-left: 5px;">Edit User&nbsp;</label>
					</div>
					<br/>
					<br/>
					<div style="position: absolute; margin: 0 auto; width: inherit;">
					...
					<div style="position: absolute; margin: 0 auto; width: inherit;">
						...
						<input type="button" class="button-color" value="     Submit     " id="editUserBtn" style = "margin-left: 140px;"/>
